<script setup>
// ESSAI : page de connexion aux couleurs de la bannière SNCF Réseau (violet, pourpre, magenta en accent).
// Même contenu que login.vue, écrit uniquement en classes Tailwind (tokens magenta-* et magenta-* dans main.css).
...
useHead({
  title: 'H00 - Login (essai)',
  description: 'Page de connexion, essai aux couleurs SNCF Réseau'
})
...
const FEUILLE =
  'transform-fill origin-center rotate-(--r) animate-settle [animation-delay:calc(var(--i)*120ms_+_var(--d)_+_150ms)] [filter:url(#login-v2-ombre)] motion-reduce:animate-none'
...
<template>
  <div
    ...
    <svg width="0" height="0" class="absolute" aria-hidden="true">
      <defs>
        <filter id="login-v2-ombre" x="-30%" y="-30%" width="160%" height="170%">
    ...
    <aside
      class="via-magenta-500 to-magenta-700 dark:from-magenta-800 dark:via-magenta-900 dark:to-magenta-950 relative flex flex-col overflow-hidden bg-linear-160 from-pink-900 px-6 pt-5 pb-12 text-white [clip-path:polygon(0_0,100%_0,100%_calc(100%-2.5rem),0_100%)] lg:sticky lg:top-0 lg:h-dvh lg:px-6 lg:py-10 lg:[clip-path:polygon(0_0,100%_0,84%_100%,0_100%)]">
      <!-- Reflet du magenta vif, en haut à gauche comme sur la bannière -->
      <div
        class="bg-magenta-500/45 dark:bg-magenta-500/20 pointer-events-none absolute -top-24 -left-24 size-80 rounded-full blur-3xl"
        aria-hidden="true" />
    ...
    <main class="px-6 pt-8 pb-14 sm:px-10 lg:flex lg:flex-col lg:justify-center lg:px-14 lg:py-16 xl:px-20">
      <div class="mx-auto w-full max-w-6xl">
        <header class="max-w-2xl">
          ...
          <div class="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:gap-5">
            <button
              ...
              class="focus-visible:outline-magenta-500 from-magenta-500 via-prune-500 to-prune-700 enabled:hover:from-magenta-700 enabled:hover:via-prune-600 enabled:hover:to-prune-800 inline-flex cursor-pointer items-center justify-center gap-2.5 rounded-lg bg-linear-90 px-4 py-3.5 text-[0.9rem] font-semibold text-white shadow-[0_8px_20px_-10px_rgb(142_9_105/0.75)] transition focus-visible:outline-2 focus-visible:outline-offset-3 enabled:active:translate-y-px disabled:cursor-wait disabled:opacity-80 sm:px-5.5 sm:text-[0.95rem] dark:shadow-none"
        ...
        <p class="mt-12 text-xs text-zinc-500 lg:hidden">v{{ APP_VERSION }} · © 2026 Applissimo</p>
      </div>
    </main>
  </div>
</template>
