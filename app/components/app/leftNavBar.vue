  ...
  <section class="w-full h-full px-4">
    ...
    >
      <div v-for="item in props.items" :key="item.value" class="border-l-muted cursor-pointer pt-1">
        ...
        >
          <Icon :name="item.icon" size="20" class="transition-colors duration-200 group-hover:text-highlighted " :class="item.value === selected ? 'text-white' : 'text-muted'" />
          <div class="text-sm font-medium transition-colors duration-200 group-hover:text-highlighted" :class="item.value === selected ? 'text-white' : 'text-gray-700'">
            {{ item.label }}
          </div>

          <div v-if="item.badge" class="ml-auto w-8 flex justify-center">
            <div class="w-full rounded border border-toned bg-accented text-xs text-center font-semibold group-hover:text-white group-hover:bg-primary-400" :class="item.value === selected ? 'text-white bg-primary-400' : 'text-gray-700'">
              {{ item.badge }}
            </div>
          </div>
        ...
        >
          <div v-for="child in item.children" :key="child.value" class="pl-4 border-l-muted border-l cursor-pointer h-9" @click.stop="selected = child.value">
            <div class="flex items-center gap-1 px-3 h-full hover:bg-accented rounded-md group" :class="child.value === selected ? 'bg-elevated' : ''">
              <div class="text-sm font-medium transition-colors duration-200 group-hover:text-highlighted" :class="child.value === selected ? 'text-highlighted' : 'text-muted'">
                {{ child.label }}
              </div>
              <div v-if="child.badge" class="ml-auto w-8 flex justify-center">
                <div class="w-full rounded border border-elevated bg-accented text-xs text-center font-semibold">
                  {{ child.badge }}
                </div>
              </div>
            </div>
