...
const inputValue = computed({
  ...
  set(value) {
    const newDate = new Date(value).toLocaleDateString("fr");
    const result = newDate.split("/").reverse().join("-");
...
<template>
  <client-only>
    <VueDatePicker class="" :placeholder="props.placeholder" locale="fr" :enable-time-picker="false" auto-apply :format="format" month-name-format="long" :week-numbers="{ type: 'iso' }" :value="props.modelValue" v-model="inputValue" />
  </client-only>
</template>
...
:root {
  ...
  --dp-month-year-row-height: 75px; /*Height of the month-year select row*/
  ...
  /*Font sizes*/
  --dp-font-size: 0.85rem; /*Default font-size*/
