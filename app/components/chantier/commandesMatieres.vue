...
<template>
  <div class="flex h-full overflow-hidden">
    ...
    <aside class="flex w-64 flex-none flex-col border-r border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      <!-- Header -->
      ...
      <template v-else>
        ...
        <div class="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-900">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <Icon :name="showCatalogue ? 'lucide:panel-right-close' : 'lucide:package-plus'" size="16" />
              {{ showCatalogue ? 'Fermer le catalogue' : 'Ajouter un article' }}
            </button>
          </div>
        </div>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/60">
                  <th class="px-4 py-3 text-left text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">N° Symbole</th>
                  <th class="px-4 py-3 text-left text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">Désignation</th>
                  <th class="px-4 py-3 text-center text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">UD</th>
                  <th class="w-28 px-4 py-3 text-center text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">Quantité</th>
                  <th class="px-4 py-3 text-right text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">Prix unit.</th>
                  <th class="px-4 py-3 text-right text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">Total</th>
                  <th class="px-4 py-3 text-left text-xs font-medium tracking-wide text-gray-500 dark:text-gray-400">Notes</th>
              ...
              <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                ...
                >
                  <td class="px-4 py-3">
                    <span class="font-mono text-xs font-semibold text-blue-600 dark:text-blue-400">{{ ligne.numero_symbole }}</span>
                  </td>
                  <td class="px-4 py-3">
                    <p class="text-xs text-gray-700 dark:text-gray-200">{{ ligne.catalogue_matieres?.description || '—' }}</p>
                    <p v-if="ligne.catalogue_matieres?.famille" class="mt-0.5 text-xs text-gray-400">{{ ligne.catalogue_matieres.famille }}</p>
                  </td>
                  <td class="px-4 py-3 text-center">
                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ ligne.catalogue_matieres?.unite_distribution || '—' }}</span>
                  </td>
                  <td class="px-4 py-3 text-center">
                    <input
                      type="number"
                  ...
                  <td class="px-4 py-3 text-right text-xs text-gray-600 dark:text-gray-300">{{ fmtPrix(ligne.catalogue_matieres?.prix_ud) }}</td>
              ...
              <tfoot>
                <tr class="border-t border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/60">
                  <td colspan="5" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Total estimé</td>
                  <td class="px-4 py-3 text-right text-sm font-bold text-gray-800 dark:text-white">{{ fmtPrix(totalEstime) }}</td>
                  <td colspan="2"></td>
                </tr>
              </tfoot>
            </table>
    ...
    <AppModal v-model="showDeleteCommande" size="sm">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-300">
        Êtes-vous sûr de vouloir supprimer <strong>« {{ commandeToDelete?.nom }} »</strong> et tous ses articles ? Cette action est irréversible.
      </p>
      <template #footer>
        <div class="flex justify-end gap-3">
          <button
