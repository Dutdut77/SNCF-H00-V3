...
<template>
  <div class="flex flex-col w-full h-full gap-4">
    ...
    <div class="flex flex-col lg:flex-row gap-4 items-center w-full justify-between">
      <AppInputSearch v-model="globalFilter" class="w-full max-w-md" size="lg" placeholder="Rechercher un utilisateur ..." />
        ...
        <AppSlideOverContent v-if="openAdd" :closeSideModal="closeAddSlide">
          ...
          <template #header>
            <div class="text-center">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">L'utilisateur pourra se connecter avec son compte SNCF</p>
            </div>
          </template>
          ...
          <template #default>
            <form @submit.prevent="ajouterUser" class="flex flex-col gap-5 w-full">
              ...
              <div class="flex flex-col gap-4 pt-2">
                <AppSwitch 
                  v-model="newUser.pre_op" 
                  name="new_pre_op" 
                  label="Pré-Op" 
                />
                <AppSwitch 
                  v-model="newUser.ref_du_rdu" 
                  name="new_ref_du_rdu" 
                  label="Référent du RDU" 
                />
              </div>
