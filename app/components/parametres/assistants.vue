  ...
  <div class="flex h-full overflow-hidden">
    ...
    <div class="flex min-h-0 flex-1 overflow-hidden">
      ...
      <aside class="flex w-72 flex-none flex-col border-r border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
        ...
        <div class="flex-none space-y-2 p-2.5">
          ...
          <div class="relative">
            ...
            <input
              ...
              class="w-full rounded-md border border-gray-200 bg-white py-1.5 pl-8 pr-2 text-sm text-gray-700 outline-none transition focus:border-blue-300 focus:ring-1 focus:ring-blue-200 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:placeholder-gray-500" />
          </div>
        </div>
        ...
        <div v-else-if="logiques.length === 0" class="flex flex-col items-center gap-3 px-4 py-12 text-center">
          <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 dark:bg-gray-800">
            <Icon name="lucide:workflow" size="22" class="text-gray-400" />
          </div>
          <div class="space-y-1">
            <p class="text-sm font-medium text-gray-600 dark:text-gray-300">Aucune logique</p>
            <p class="text-sm text-gray-400">Crée ta première logique métier</p>
          </div>
        </div>
      ...
      <main class="flex min-w-0 flex-1 flex-col overflow-hidden">
        <div v-if="!selectedLogique" class="flex h-full flex-col items-center justify-center gap-3 text-gray-400">
    ...
    <AppModal v-model="showFormLogique" size="md">
      ...
      <form class="space-y-4" @submit.prevent="submitLogique">
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Icône (Lucide)</label>
          <div class="flex items-center gap-2">
            <Icon :name="formIcone" size="18" class="text-gray-500" />
            <input
              v-model="formIcone"
              type="text"
              placeholder="lucide:workflow"
              class="flex-1 rounded-lg border border-gray-200 px-3 py-2 font-mono text-sm text-gray-800 transition outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
          </div>
          <p class="mt-1 text-xs text-gray-400">Nom d'icône Lucide (ex : lucide:train-track, lucide:zap, lucide:plug)</p>
        </div>
      </form>
      <template #footer>
    ...
    <AppModal v-model="showDeleteLogique" size="sm">
      <template #header>
        <h3 class="text-base font-semibold text-gray-800 dark:text-white">Supprimer la logique</h3>
      </template>
