...
const rp1Switch = computed({
  ...
  set: (val) => { tache.value.rp1 = val ? 1 : 0; }
});
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            >
              ...
              <td class="px-4 py-3">
                <div class="flex items-start gap-3">
                  <div class="w-8 h-8 rounded-lg bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                  ...
                  <div class="flex flex-col min-w-0">
                    ...
                    <span class="text-xs text-gray-500 dark:text-gray-400 md:hidden mt-1">
            ...
            <tr v-if="filteredTaches.length === 0">
              <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                <Icon name="lucide:clipboard-x" class="w-8 h-8 mx-auto mb-2 opacity-50" />
          ...
          <template #header>
            <div class="text-center">
              <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center">
                <Icon :name="isNewTache ? 'lucide:clipboard-plus' : 'lucide:clipboard-edit'" size="28" class=" text-primary-500" />
              </div>
              <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
                {{ isNewTache ? 'Nouvelle tâche' : 'Modifier la tâche' }}
              </h2>
              <p v-if="!isNewTache" class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                ID: {{ tache.id }}
              </p>
            </div>
          </template>
          ...
          <template #default>
            <form @submit.prevent="enregistrer" class="flex flex-col gap-5 w-full">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                />
                <p v-if="!isNewTache" class="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
    ...
    <AppModal v-model="showDeleteModal" size="md" :persistent="isDeleting" @close="cancelDelete">
      ...
      <template #default>
        <p class="text-center text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
          ...
          <span class="font-semibold text-gray-900 dark:text-white">« {{ tacheToDelete?.tache?.substring(0, 50) || '' }}{{ tacheToDelete?.tache?.length > 50 ? '...' : '' }} »</span> ? 
