<script setup>
import { changelog, APP_VERSION } from '~/utils/changelog'

const { checkUserViewedUpdate, markUpdateAsViewed, isUpdateVisible, pendingReleases } = useUpdate()
const user = useAuthUser()

// Vérification initiale : quelle version l'utilisateur a-t-il déjà validée ?
await checkUserViewedUpdate(user.value?.email)

// Photo des nouveautés au moment de l'ouverture : `pendingReleases` se vide dès
// le clic sur « C'est noté », les badges ne doivent pas disparaître pendant la
// transition de fermeture.
const newVersions = ref(new Set())

watch(
  isUpdateVisible,
  (visible) => {
    if (visible) newVersions.value = new Set(pendingReleases.value.map((release) => release.version))
  },
  { immediate: true }
)
          ...
          <Transition
            ...
            <div
              ...
              <div class="flex-1 space-y-8 overflow-auto p-6">
                <div v-for="(update, index) in changelog" :key="update.version" class="relative">
                  <!-- Ligne de timeline -->
                  <div
                  ...
                  <div class="flex gap-4">
                    ...
                    <div class="flex-1 pb-6">
                      <div class="mb-3 flex items-center gap-3">
                        ...
                        <span
                          v-if="newVersions.has(update.version)"
              ...
              <div class="border-primary-200 bg-primary-50 border-t p-6">
                <div class="flex items-center justify-between">
                  <p class="text-primary-400 text-xs">Dernière mise à jour : {{ changelog[0]?.date }}</p>
