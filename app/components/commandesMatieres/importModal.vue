...
const { createCommande, addLigne } = useCommandesMatieres()
...
const inconnus  = ref([])   // [{ symbole, quantite }]

// Stocker les rows parsés
...
const parseFile = (file) => {
  ...
  reader.onload = (ev) => {
    ...
    // Lire en tableau brut pour trouver la ligne d'en-têtes (qui peut ne pas être la 1ère)
    const raw = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' })

    // Chercher la ligne qui contient "NUMERO" dans l'une de ses cellules
    ...
    // Reconstruire un tableau d'objets à partir de la ligne d'en-têtes trouvée
...
const doImport = async () => {
  if (!listNom.value.trim() || !reconnus.value.length) return
  ...
  const lignes = []
  for (let i = 0; i < reconnus.value.length; i++) {
    const { symbole, quantite } = reconnus.value[i]
...
<template>
  <AppModal :model-value="open" size="lg" @update:model-value="emit('close')">
    ...
    <div v-else class="space-y-4">
      ...
      <!-- Tableau reconnus -->
      <div v-if="activeTab === 'reconnus'" class="max-h-52 overflow-auto rounded-lg border border-gray-200 dark:border-gray-700">
      ...
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex gap-3">
          ...
          <button
            v-else
            type="button"
            :disabled="!reconnus.length || !listNom.trim() || importing"
            class="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-700 disabled:opacity-50"
            @click="doImport"
          >
            ...
            Importer {{ reconnus.length }} article{{ reconnus.length > 1 ? 's' : '' }}
          </button>
        </div>
      </div>
    </template>
