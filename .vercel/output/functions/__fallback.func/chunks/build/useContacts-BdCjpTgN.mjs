...
const useContacts = () => {
  ...
  const getContactsGeneralites = async (chantierId) => {
    try {
      const { data, error } = await client.from("chantier_contacts_generalites").select("*").eq("chantier_id", chantierId).single();
  ...
  const upsertContactsGeneralites = async (chantierId, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_generalites").upsert(
        {
  ...
  const getContactsTravaux = async (chantierId) => {
    try {
      const { data, error } = await client.from("chantier_contacts_travaux").select("*").eq("chantier_id", chantierId).single();
      if (error && error.code !== "PGRST116") throw error;
      return data;
    } catch (err) {
      addToast({
  ...
  const upsertContactsTravaux = async (chantierId, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_travaux").upsert(
        {
          chantier_id: chantierId,
  ...
  const getContactsEntreprises = async (chantierId) => {
    try {
      const { data, error } = await client.from("chantier_contacts_entreprises").select("*").eq("chantier_id", chantierId).order("created_at", { ascending: true });
      if (error) throw error;
      return data || [];
    } catch (err) {
      addToast({
  ...
  const addContactEntreprise = async (chantierId, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_entreprises").insert({
        chantier_id: chantierId,
  ...
  const updateContactEntreprise = async (id, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_entreprises").update({
        metier: contactData.metier || null,
        entreprise: contactData.entreprise || null,
        responsable_nom: contactData.responsable_nom || null,
        responsable_email: contactData.responsable_email || null,
  ...
  const getContactsEtudes = async (chantierId) => {
    try {
      const { data, error } = await client.from("chantier_contacts_etudes").select("*").eq("chantier_id", chantierId).single();
      if (error && error.code !== "PGRST116") throw error;
      return data;
    } catch (err) {
      addToast({
  ...
  const upsertContactsEtudes = async (chantierId, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_etudes").upsert(
        {
          chantier_id: chantierId,
  ...
  const getContactsAutres = async (chantierId) => {
    try {
      const { data, error } = await client.from("chantier_contacts_autres").select("*").eq("chantier_id", chantierId).order("created_at", { ascending: true });
      if (error) throw error;
      return data || [];
    } catch (err) {
      addToast({
  ...
  const addContactAutre = async (chantierId, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_autres").insert({
        chantier_id: chantierId,
        metier: contactData.metier || null,
        entreprise: contactData.entreprise || null,
        responsable_nom: contactData.responsable_nom || null,
  ...
  const updateContactAutre = async (id, contactData) => {
    try {
      const { data, error } = await client.from("chantier_contacts_autres").update({
        metier: contactData.metier || null,
        entreprise: contactData.entreprise || null,
        responsable_nom: contactData.responsable_nom || null,
        responsable_email: contactData.responsable_email || null,
...
export { useContacts as u };
//# sourceMappingURL=useContacts-BdCjpTgN.mjs.map
