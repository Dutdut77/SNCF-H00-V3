...
const useChantiers = () => {
  ...
  const getChantiers = async () => {
    try {
      const { data, error } = await supabase.from("chantiers").select("id, compte, name, ligne_id, matiere, matiere_da, etat, lignes(id, name), date_rea, date_prepa, autre");
      if (error) {
  ...
  const updateChantier = async (id, updates) => {
    try {
      const { data, error } = await supabase.from("chantiers").update(updates).eq("id", id).select(
        "id, compte, name, ligne_id, date_rea, date_prepa, etat, type_essais, decret, matiere, matiere_da, compte_moe, compte_slg, compte_matieres, autre, lignes(id, name)"
      ).maybeSingle();
      if (error) throw error;
      addToast({
...
export { useChantiers as u };
//# sourceMappingURL=useChantiers-C2XRmo5v.mjs.map
