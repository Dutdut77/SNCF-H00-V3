...
const useTimeline = () => {
  ...
  const getAllWeekends = async () => {
    try {
      const { data, error } = await supabase.from("timeline").select("*").eq("type", "weekend").order("annee_debut", { ascending: true }).order("semaine_debut", { ascending: true });
  ...
  const getTimelineByChantier = async (chantierId) => {
    try {
      const { data, error } = await supabase.from("timeline").select("*").eq("chantier_id", chantierId).order("annee_debut", { ascending: true }).order("semaine_debut", { ascending: true });
      if (error) {
  ...
  const getWeekendsByChantier = async (chantierId) => {
    try {
      const { data, error } = await supabase.from("timeline").select("*").eq("chantier_id", chantierId).eq("type", "weekend").order("annee_debut", { ascending: true }).order("semaine_debut", { ascending: true });
      if (error) {
        console.error("Erreur Supabase:", error);
        throw error;
      }
  ...
  const replaceWeekendsForChantier = async (chantierId, weekends) => {
    try {
      const { error: deleteError } = await supabase.from("timeline").delete().eq("chantier_id", chantierId).eq("type", "weekend");
...
export { useTimeline as u };
//# sourceMappingURL=useTimeline-BdoQfffy.mjs.map
